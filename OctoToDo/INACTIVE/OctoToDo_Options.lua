...
MAIN_TITLE:SetText(E.func_AddonVersion(GlobalAddonName))
...
local function Create_SimpleButton(scroll, self, number, pos, config, text, color)
	...
	if not self[number..pos..config] then
		...
		self[number..pos..config]:SetScript("OnClick", function(btn)
				if config == "OctoToDo_DB_Config" then
					OctoToDo_DB_Config = {}
					ReloadUI()
				end
				if config == "OctoToDo_DB_Other" then
					OctoToDo_DB_Other = {}
					ReloadUI()
				end
				if config == "OctoToDo_DB_Levels" then
					OctoToDo_DB_Levels = {}
					ReloadUI()
				end
				if config == "OctoToDo_DB_Vars" then
					OctoToDo_DB_Vars = {}
					ReloadUI()
				end
...
MAIN_Config:SetScript("OnShow", function(self)
		...
		btn_right1:ddSetMenuButtonHeight(16)
		number = 2
		...
		btn_right2:ddSetInitFunc(function(self, level, value)
				...
				self:ddAddSeparator(level)
				if not value then
					for k, v in ipairs(E.OctoTable_Prefixes) do
						info.isNotRadio = true
						info.notCheckable = false
						info.text = v
						info.value = k
		...
		btn_right2:ddSetMenuButtonHeight(16)
		...
		Create_CheckButton(MAIN_scrollChild, self, 27, POS_RIGHT, 0, "OctoToDo_debug_Function_FIRST", E.func_texturefromIcon(E.AddonTexture_1, indent)..E.Gray_Color.."Debug".."|r")
		...
		Create_SimpleButton(MAIN_scrollChild, self, 31, POS_RIGHT, "OctoToDo_DB_Config", E.Gray_Color.."Config".."|r")
		Create_SimpleButton(MAIN_scrollChild, self, 32, POS_RIGHT, "OctoToDo_DB_Other", E.Gray_Color.."Other".."|r")
		Create_SimpleButton(MAIN_scrollChild, self, 33, POS_RIGHT, "OctoToDo_DB_Levels", E.Gray_Color.."Players".."|r")
		Create_SimpleButton(MAIN_scrollChild, self, 34, POS_RIGHT, "OctoToDo_DB_Vars", E.Gray_Color.."Vars".."|r")
...
local category, layout = Settings.RegisterCanvasLayoutCategory(MAIN_Config, E.func_AddonTitle(GlobalAddonName).." OLD")
category.ID = E.func_AddonTitle(GlobalAddonName).." OLD"
...
TITLE_FIRST:SetText(E.func_AddonVersion(GlobalAddonName))
...
TITLE_SECOND:SetText(E.func_AddonVersion(GlobalAddonName))
...
TITLE_THIRD:SetText(E.func_AddonVersion(GlobalAddonName))
...
TITLE_FOURTH:SetText(E.func_AddonVersion(GlobalAddonName))
...
TITLE_FIFTH:SetText(E.func_AddonVersion(GlobalAddonName))
...
-- TITLE_SIXTH:SetText(E.func_AddonVersion(GlobalAddonName))
...
TITLE_SEVENTH:SetText(E.func_AddonVersion(GlobalAddonName))
