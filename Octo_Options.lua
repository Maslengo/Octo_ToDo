...
config_MAIN:SetScript("OnShow", function(self)
		...
		-- title_MAIN
		if not title_MAIN then
			title_MAIN = self:CreateFontString(nil, "ARTWORK", "GameFontHighlightSmall")
		end
		title_MAIN:SetPoint("TOPLEFT", 4, 30)
		title_MAIN:SetText(C_AddOns.GetAddOnMetadata(GlobalAddonName, "Version"))
		title_MAIN:SetTextColor(.5, .5, .5, 1)
		--
		local number = 1
		...
		self[pizda]:SetPoint("TOPLEFT", title_MAIN, "BOTTOMLEFT", POS_LEFT, -indent*number)
		...
		self[pizda]:SetPoint("TOPLEFT", title_MAIN, "BOTTOMLEFT", POS_CENTER, -indent*number)
		...
		self[pizda]:SetWidth(140)
		self[pizda]:SetPoint("TOPLEFT", title_MAIN, "BOTTOMLEFT", POS_RIGHT, -indent*number)
		self[pizda]:SetMinMaxValues(60, 160)
		local step = 2
		self[pizda]:SetValueStep(step)
		...
		local number = 12
		local pizda = E.Octo_Func.GenerateUniqueID()
		if not curWidthTitleAchievementTEXT then
			curWidthTitleAchievementTEXT = self:CreateFontString(nil, "ARTWORK", "GameFontNormalSmall")
		end
		if not self[pizda] then
			self[pizda] = CreateFrame("Slider", nil, self, "OptionsSliderTemplate")
		end
		...
		local step = 5
		self[pizda]:SetValueStep(step)
		self[pizda]:SetValue(Octo_ToDoVars.config.curWidthTitleAchievement)
		self[pizda]:SetScript("OnValueChanged", function (self, value)
				value = math.floor(value * (1 / step) + .5) / (1 / step)
		...
		curWidthTitleAchievementTEXT:SetPoint("BOTTOMLEFT", self[pizda], "TOPLEFT", indent, 0)
...
-----
-----
config_SECOND = CreateFrame("FRAME", GlobalAddonName.."config_SECOND"..E.Octo_Func.GenerateUniqueID())
config_SECOND:Hide()
config_SECOND:SetScript("OnShow", function(self)
		self:SetPoint("TOPLEFT", 0, 0)
		-- TITLE
		if not title_SECOND then
			title_SECOND = self:CreateFontString(nil, "ARTWORK", "GameFontHighlightSmall")
		end
		...
		for k, repID in ipairs(E.Octo_Table.OctoTable_reputation_BC) do
			...
			self[pizda]:SetPoint("TOPLEFT", title_SECOND, "BOTTOMLEFT", POS_LEFT, -indent*number)
			self[pizda]:SetChecked(Octo_ToDoVars.config.reputation[repID])
			self[pizda]:SetScript("OnClick", function(btn)
					Octo_ToDoVars.config.reputation[repID] = btn:GetChecked()
					StaticPopup_Show(GlobalAddonName.."GET_RELOAD")
			end)
			self[pizda].text:SetText(E.Octo_Func.func_reputationName(repID))
		end
end)
-- ADD SUBCATEGORY
local subcategory, layout = Settings.RegisterCanvasLayoutSubcategory(category, config_SECOND, "Burning Crusade")
subcategory.ID = "Burning Crusade"
Settings.RegisterAddOnCategory(subcategory)
-----
-----
-----
config_THIRD = CreateFrame("FRAME", GlobalAddonName.."config_THIRD"..E.Octo_Func.GenerateUniqueID())
config_THIRD:Hide()
config_THIRD:SetScript("OnShow", function(self)
		self:SetPoint("TOPLEFT", 0, 0)
		-- TITLE
		if not title_THIRD then
			title_THIRD = self:CreateFontString(nil, "ARTWORK", "GameFontHighlightSmall")
		end
		...
		for k, repID in ipairs(E.Octo_Table.OctoTable_reputation_Classic) do
			local number = k
			local pizda = E.Octo_Func.GenerateUniqueID()
			if not self[pizda] then
				self[pizda] = CreateFrame("CheckButton", nil, self, "InterfaceOptionsCheckButtonTemplate")
			end
			self[pizda]:SetPoint("TOPLEFT", title_THIRD, "BOTTOMLEFT", POS_LEFT, -indent*number)
			self[pizda]:SetChecked(Octo_ToDoVars.config.reputation[repID])
			self[pizda]:SetScript("OnClick", function(btn)
					Octo_ToDoVars.config.reputation[repID] = btn:GetChecked()
					StaticPopup_Show(GlobalAddonName.."GET_RELOAD")
...
-- ADD SUBCATEGORY
local subcategory, layout = Settings.RegisterCanvasLayoutSubcategory(category, config_THIRD, "Classic")
subcategory.ID = "Classic"
Settings.RegisterAddOnCategory(subcategory)
