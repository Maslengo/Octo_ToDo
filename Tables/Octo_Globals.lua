...
local LibStub, ldb, ldbi = LibStub, LibStub("LibDataBroker-1.1"),
LibStub("LibDBIcon-1.0")
...
local function texturefromIcon(self, size)
	if show == true then
		...
		if not self or self == 0 then self = 134400 end
		return "|T" .. self .. ":" .. size .. ":" .. size ..
		":::64:64:4:60:4:60|t"
	else
		return ""
	end
...
if IsRetail() then E.baseWowheadUrl = "https://wowhead.com/%s%s=%s" end
if IsClassic() then E.baseWowheadUrl = "https://wowhead.com/classic/%s=%s%s" end
if IsCata() then E.baseWowheadUrl = "https://wowhead.com/cata/%s=%s%s" end
...
E.regions = {[1] = "us", [2] = "kr", [3] = "eu", [4] = "tw", [5] = "cn"}
