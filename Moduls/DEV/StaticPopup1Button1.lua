    ...
    if Octo_ToDo_DB_Vars.config.StaticPopup1Button1 then
        ...
        function Octo_OnLoad()
            ...
            if not EventFrame then
                EventFrame = CreateFrame("FRAME", AddonTitle ..
                                             E.Octo_Func.GenerateUniqueID())
            end
        ...
        function Octo_OnEvent(self, event, ...)
            if (event == "EQUIP_BIND_CONFIRM" or event ==
                "EQUIP_BIND_REFUNDABLE_CONFIRM") and not InCombatLockdown() then
                accept()
            end
        end
        Octo_OnLoad()
