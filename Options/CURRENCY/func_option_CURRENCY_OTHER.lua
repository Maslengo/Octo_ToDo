...
function E.func_option_CURRENCY_OTHER(category, layout)
	...
	----------------------------------------------------------------
	-- 2. Octo_ToDo_DB_Vars.CONFIG_CURRENCY_SHOW_BRACKETS ----------
	----------------------------------------------------------------
	do
		...
		tooltip = tooltip.."|n|n"..L["Shows earned amount in () if currency has weekly/seasonal cap"]
		local initializer = Settings.CreateCheckbox(category, setting, tooltip)
	end
	----------------------------------------------------------------
	-- 3. Octo_ToDo_DB_Vars.CONFIG_CURRENCY_COLOR_BRACKETS ----------
	----------------------------------------------------------------
	do
		local name = COLOR
	...
	----------------------------------------------------------------
	-- 4. Octo_ToDo_DB_Vars.CONFIG_CURRENCY_SHOW_REMAINING ----------
	----------------------------------------------------------------
	do
		local variable = E.func_GenerateID()
		...
		setting:SetValueChangedCallback(E.func_UpdateGlobals)
		local tooltip = E.func_defaultValue_tooltip(defaultValue)
		tooltip = tooltip.."|n|n"..L["Shows remaining amount if currency has weekly/seasonal cap"]
		local initializer = Settings.CreateCheckbox(category, setting, tooltip)
	end
	----------------------------------------------------------------
	-- 5. Octo_ToDo_DB_Vars.CONFIG_CURRENCY_COLOR_REMAINING ----------
	----------------------------------------------------------------
	do
		local name = COLOR
