local GlobalAddonName, E = ...
local L = E.L
----------------------------------------------------------------
E.OctoTable_Expansions = {
	[1] = {
		color = E.COLOR_WORLDOFWARCRAFT, --E.func_RGB2Hex(EXPANSION_COLOR_1.r, EXPANSION_COLOR_1.g, EXPANSION_COLOR_1.b)
		name = "World of Warcraft",
		nameBlizzard = L["EXPANSION_NAME0"] or "World of Warcraft", -- UNUSED
		nameVeryShort = "WoW",
		icon = 1385726,
		iconOLD = 254094, -- UNUSED
		id = 1,
		timewalkDungeonID = 2634,
	},
	[2] = {
		color = E.COLOR_THEBURNINGCRUSADE, --E.func_RGB2Hex(EXPANSION_COLOR_2.r, EXPANSION_COLOR_2.g, EXPANSION_COLOR_2.b)
		name = "The Burning Crusade",
		nameBlizzard = L["EXPANSION_NAME1"] or "The Burning Crusade", -- UNUSED
		nameVeryShort = "TBC",
		icon = 1378987,
		iconOLD = 236415, -- UNUSED
		id = 2,
		timewalkDungeonID = 744,
	},
	[3] = {
		color = E.COLOR_WRATHOFTHELICHKING, --E.func_RGB2Hex(EXPANSION_COLOR_3.r, EXPANSION_COLOR_3.g, EXPANSION_COLOR_3.b)
		name = "Wrath of the Lich King",
		nameBlizzard = L["EXPANSION_NAME2"] or "Wrath of the Lich King", -- UNUSED
		nameVeryShort = "WotLK",
		icon = 607688,
		iconOLD = 341221, -- UNUSED
		id = 3,
		timewalkDungeonID = 995,
	},
	[4] = {
		color = E.COLOR_CATACLYSM, --E.func_RGB2Hex(EXPANSION_COLOR_4.r, EXPANSION_COLOR_4.g, EXPANSION_COLOR_4.b)
		name = "Cataclysm",
		nameBlizzard = L["EXPANSION_NAME3"] or "Cataclysm", -- UNUSED
		nameVeryShort = "Cata",
		icon = 536055,
		iconOLD = 574788, -- UNUSED
		id = 4,
		timewalkDungeonID = 1146,
	},
	[5] = {
		color = E.COLOR_MISTSOFPANDARIA, --E.func_RGB2Hex(EXPANSION_COLOR_5.r, EXPANSION_COLOR_5.g, EXPANSION_COLOR_5.b)
		name = "Mists of Pandaria",
		nameBlizzard = L["EXPANSION_NAME4"] or "Mists of Pandaria", -- UNUSED
		nameVeryShort = "MoP",
		icon = 901157,
		iconOLD = 896622, -- UNUSED
		id = 5,
		timewalkDungeonID = 1453, -- (2538 REMIX)
	},
	[6] = {
		color = E.COLOR_WARLORDSOFDRAENOR, --E.func_RGB2Hex(EXPANSION_COLOR_6.r, EXPANSION_COLOR_6.g, EXPANSION_COLOR_6.b)
		name = "Warlords of Draenor",
		nameBlizzard = L["EXPANSION_NAME5"] or "Warlords of Draenor", -- UNUSED
		nameVeryShort = "WoD",
		icon = 1134497,
		iconOLD = 1113431, -- UNUSED
		id = 6,
		timewalkDungeonID = 1971,
	},
	[7] = {
		color = E.COLOR_LEGION, --E.func_RGB2Hex(EXPANSION_COLOR_7.r, EXPANSION_COLOR_7.g, EXPANSION_COLOR_7.b)
		name = "Legion",
		nameBlizzard = L["EXPANSION_NAME6"] or "Legion", -- UNUSED
		nameVeryShort = "Legion",
		icon = 1715536,
		iconOLD = 1711335, -- UNUSED
		id = 7,
		timewalkDungeonID = 2274, -- (2817 REMIX)
	},
	[8] = {
		color = E.COLOR_BATTLEFORAZEROTH, --E.func_RGB2Hex(EXPANSION_COLOR_8.r, EXPANSION_COLOR_8.g, EXPANSION_COLOR_8.b)
		name = "Battle for Azeroth",
		nameBlizzard = L["EXPANSION_NAME7"] or "Battle for Azeroth", -- UNUSED
		nameVeryShort = "BfA",
		icon = 3256381,
		iconOLD = 3194610, -- UNUSED
		id = 8,
		timewalkDungeonID = 2874,
	},
	[9] = {
		color = E.COLOR_SHADOWLANDS, --E.func_RGB2Hex(EXPANSION_COLOR_9.r, EXPANSION_COLOR_9.g, EXPANSION_COLOR_9.b)
		name = "Shadowlands",
		nameBlizzard = L["EXPANSION_NAME8"] or "Shadowlands", -- UNUSED
		nameVeryShort = "SL",
		icon = 4465334,
		iconOLD = 4254080, -- UNUSED
		id = 9,
		-- timewalkDungeonID = 0,
	},
	[10] = {
		color = E.COLOR_DRAGONFLIGHT, --E.func_RGB2Hex(EXPANSION_COLOR_10.r, EXPANSION_COLOR_10.g, EXPANSION_COLOR_10.b)
		name = "Dragonflight",
		nameBlizzard = L["EXPANSION_NAME9"] or "Dragonflight", -- UNUSED
		nameVeryShort = "DF",
		icon = 5409250,
		iconOLD = 5342925, -- UNUSED
		id = 10,
		-- timewalkDungeonID = 0,
	},
	[11] = {
		color = E.COLOR_THEWARWITHIN, --E.func_RGB2Hex(EXPANSION_COLOR_11.r, EXPANSION_COLOR_11.g, EXPANSION_COLOR_11.b)
		name = "The War Within",
		nameBlizzard = L["EXPANSION_NAME10"] or "The War Within", -- UNUSED
		nameVeryShort = "TWW",
		icon = 6980554,
		iconOLD = 5779391, -- UNUSED
		id = 11,
		-- timewalkDungeonID = 0,
	},
	[12] = {
		color = E.COLOR_MIDNIGHT, --E.func_RGB2Hex(EXPANSION_COLOR_12.r, EXPANSION_COLOR_12.g, EXPANSION_COLOR_12.b)
		name = "Midnight",
		nameBlizzard = L["EXPANSION_NAME11"] or "Midnight", -- UNUSED
		nameVeryShort = "MN",
		icon = 7852832, -- 7455547, -- [[Interface\EncounterJournal\UI-EJ-BOSS-Default]]
		iconOLD = 132123,
		id = 12,
		-- timewalkDungeonID = 0,
	},
	[13] = {
		color = E.COLOR_THELASTTITAN, --E.func_RGB2Hex(EXPANSION_COLOR_13.r, EXPANSION_COLOR_13.g, EXPANSION_COLOR_13.b)
		name = "The Last Titan",
		nameBlizzard = L["EXPANSION_NAME12"] or "The Last Titan", -- UNUSED
		nameVeryShort = "TLT",
		icon = [[Interface\EncounterJournal\UI-EJ-BOSS-Default]],
		id = 13,
		iconOLD = 4622478,
		-- timewalkDungeonID = 0,
	},
}