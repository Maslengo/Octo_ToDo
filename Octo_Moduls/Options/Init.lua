...
function EventFrame:func_InitOptions()
	if not E.func_Octo_Moduls_Options then return end
	local name = E.func_AddonNameForOptionsInit(GlobalAddonName)
	local parent = E.func_GetAddOnMetadata(E.MainAddonName, "Title")
	LibStub("AceConfigRegistry-3.0"):RegisterOptionsTable(name, E.func_Octo_Moduls_Options(EventFrame.savedVars))
