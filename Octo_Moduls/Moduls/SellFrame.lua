...
----------------------------------------------------------------
local EventFrame = CreateFrame("Frame")
EventFrame.BagAndSlot = {} -- Таблица для хранения слотов предметов для продажи
...
if not EventFrame.promise then
	EventFrame.promise = LibThingsLoad:Items(E.OctoTable_itemID_Ignore_List)
end
...
function EventFrame.func_CreateBankButtons()
...
function EventFrame:TOOLTIP_DATA_UPDATE(dataInstanceID)
	...
	for bag = BACKPACK_CONTAINER, NUM_TOTAL_EQUIPPED_BAG_SLOTS do
		...
		for slot = numSlots, 1, -1 do
			...
			if containerInfo and not ignorelist[containerInfo.itemID] then
				...
				then
					table.insert(EventFrame.BagAndSlot, {bag, slot})
...
function EventFrame.func_SellItemsByQuality()
	-- Защита от повторного клика
	if EventFrame.sellLock then return end
	EventFrame.sellLock = true
	-- Обновляем список предметов перед продажей
	func_UpdateTooltip(activeTooltipButton)
	local totalItems = #EventFrame.BagAndSlot
	if totalItems == 0 then
		-- print("Нет предметов для продажи.")
		EventFrame.sellLock = false
		return
	end
	-- Создаем свой фрейм для отображения прогресса
	if not EventFrame.progressFrame then
		EventFrame.progressFrame = CreateFrame("Frame", nil, UIParent, "BackdropTemplate")
		EventFrame.progressFrame:SetSize(300, 80)
		EventFrame.progressFrame:SetPoint("CENTER")
		EventFrame.progressFrame:SetBackdrop({
			bgFile = "Interface\\DialogFrame\\UI-DialogBox-Background",
			edgeFile = "Interface\\DialogFrame\\UI-DialogBox-Border",
			tile = true, tileSize = 32, edgeSize = 32,
			insets = { left = 11, right = 12, top = 12, bottom = 11 }
		})
		EventFrame.progressFrame.text = EventFrame.progressFrame:CreateFontString()
		EventFrame.progressFrame.text:SetFontObject(OctoFont11)
		EventFrame.progressFrame.text:SetPoint("TOP", 0, -15)
		EventFrame.progressFrame.text:SetText("Продажа предметов...")
		EventFrame.progressFrame.progressText = EventFrame.progressFrame:CreateFontString()
		EventFrame.progressFrame.progressText:SetFontObject(OctoFont11)
		EventFrame.progressFrame.progressText:SetPoint("CENTER", 0, 0)
		EventFrame.progressFrame.cancelButton = CreateFrame("Button", nil, EventFrame.progressFrame, "UIPanelButtonTemplate")
		EventFrame.progressFrame.cancelButton:SetSize(100, 25)
		EventFrame.progressFrame.cancelButton:SetPoint("BOTTOM", 0, 10)
		EventFrame.progressFrame.cancelButton:SetText(CANCEL)
		EventFrame.progressFrame.cancelButton:SetScript("OnClick", function()
			EventFrame.sellLock = false
			EventFrame.progressFrame:Hide()
		end)
	end
	-- Показываем наш фрейм прогресса
	EventFrame.progressFrame.progressText:SetText(TIME_REMAINING..totalItems.."/"..totalItems)
	EventFrame.progressFrame:Show()
	-- Функция для продажи одного предмета
	local function SellNextItem()
		if #EventFrame.BagAndSlot == 0 or not EventFrame.sellLock then
		...
		local item = table.remove(EventFrame.BagAndSlot, 1)
		C_Container.UseContainerItem(item[1], item[2])
		-- Обновляем прогресс
		local currentTotal = #EventFrame.BagAndSlot
		totalItems = math.max(totalItems, currentTotal) -- обновляем максимум только если вырос
		EventFrame.progressFrame.progressText:SetText(TIME_REMAINING .. currentTotal .. "/" .. totalItems)
...
function EventFrame.func_CreateTradeButtons()
	...
	for _, i in ipairs({3, 4}) do -- 0 COMMON, 3 RARE, 4 EPIC
		...
		OctoFrame_SellOtherBlue:SetScript("OnClick", function()
				EventFrame.func_SellItemsByQuality()
		end)
...
function EventFrame:PLAYER_LOGIN()
	...
	EventFrame.func_CreateBankButtons()
end
