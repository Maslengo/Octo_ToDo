...
function lib.C_Table_Search(find)
	...
	for gname, gvalue in next, (_G)do
		if tostring(gname):match("^C_") and type(gvalue)=="table" then
			for fname in next, (_G[gname]) do
...
for k,v in next, (lib.deprecated)do
...
if WOW_PROJECT_ID~=WOW_PROJECT_MAINLINE then
	...
	for name1, funcs in next, (lib.deprecated) do
		if type(funcs)=="function" then
			if _G[name1] then
				lib.debug("deprecated-api ("..current..")",name1);
			end
		else
			for name2 in next, (funcs) do
